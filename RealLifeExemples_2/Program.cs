...
namespace RealLifeExemples_2
{
	class Program
	{
		...
		public static string SpinWords(string sentence)
		{
			...
			return new string(sentence);     // Reverse all letters's position of a word.
		...
		public static int NextId(int[] ids)
		{
			...
			return i;     // return the smallest unused data;
		}


		public static bool IsIsogram(string str)
		{
			return str.Length == new HashSet<char>(str.ToLower()).Count; // Check if a string has repeating chars, consecutive or non consecutively;
		...
		{
			return a.Sum(x => (int)Math.Pow(x, n) - x); // For the given array, we calculate the sum of each value to the n-th power. Then subtract the sum of the original array.
		}
		...
		public static int NbYear(int p0, double percent, int aug, int p)
		{
			...
			return year;     // In a small town the population is p0 at the beginning of a year. The population regularly increases by 2 percent per year and moreover aug new inhabitants per year come to live in the town. It returns how many years the town need to see its population greater or equal to p inhabitants.
		}


		public static string IsEvenOrOdd(int num)
		{
			if ((num % 2) == 0)   // Check if an int is odd or even;
			{
				return "even";
			}
			else
		...
		{                                 // 
			int i = 1;                    //
			int sum = 0;                  //  or can be the same as
			while (i <= num)              // {
			{                             //  return (num*(num+1))/2;
				sum += i;                 //  }
				i++;                     //
		...
		{
			for (int i = 0; i < array.Length; i++)    //  Given an array with positive numbers and a number n We find the N-th power of the element in the array with the index N. If n is outside of the array, then return -1.
			{
				if (i == n)
				{
		...
		{
			return word.Split('-').Length;     // Create a function that counts the number of syllables a word has when each syllable is separated with a dash.
		}
		...
		{                                          //{
			...
			int x;                                 //      int x = 0;
												   //
			x = Array.IndexOf(arr, str);   // Or         for (int i = 0; arr.Length > i; i++)
										   //      {
		...
		/*public static bool hurdleJump(int[] hurdles, int jumpHeight)
		{
            ...
            {
				return true;                            // This method is correct but it will usually output an error message : System.InvalidOperationException : Sequence contains no elements
            }
		...
		public static bool hurdleJump(int[] hurdles, int jumpHeight)
		{
			for (int i = 0; i < hurdles.Length; i++)          //Create a function that takes an array int[] hurdles and an int jumpHeight, and determine whether or not jumpHeight is greater or equal than any of hurdles value.
			{
		...
		public static int CountWords(string str)  // the function takes a string and returns the word count. The string will be a sentence.
		{                                           // {
			int count = 0;            // Or can be: //   
		...
		{
			if (s1.ToLower().Equals(s2.ToLower()))     // check if two strings are identicals regardless the upper cases they might have.
			{
				return true;
			}
		...
		{                                                  //{
														   //
			return arr.Select(Math.Abs).Sum();  //or      //  int X = 0;
												//
		...
		public static int solutions(int a, int b, int c) //A quadratic equation a x² + b x + c = 0 has either 0, 1, or 2 distinct solutions for real values of x. Given a, b and c, you should return the number of solutions to the equation.
		{    
			int x = (b * b) - (4 *a  * c);      
		...
		public static int Factorial(int num) // Create a function that takes an integer and returns the factorial of that integer. (ex: Factorial(3) ➞ 6 or Factorial(5) ➞ 120)
		{                     // Or   {                                     Or else {
			                        //                         
		...
		public static bool SameCase(string str) // Return true if a string contains only Upper characters or only lower characters.
		{
